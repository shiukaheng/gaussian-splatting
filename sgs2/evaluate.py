...
def evaluate_camera(model: GaussianModel, scene: Scene, camera: Camera):
    ...
    with torch.no_grad():
        ...
        pred = render(camera, model, model.pipeline_params, background)["render"]
...
def evaluate_scene(scene: Scene, model: GaussianModel, progress_callback: Callable[[int, int], None] = None): # current, total
    with torch.no_grad():
        ...
        for camera in tqdm(train_cameras):
            ssim_val, psnr_val, lpips_val, pred, gt = evaluate_camera(model, scene, camera)
        ...
        for camera in tqdm(test_cameras):
            ssim_val, psnr_val, lpips_val, preg, gt = evaluate_camera(model, scene, camera)
            ssim_val, psnr_val, lpips_val = ssim_val.item(), psnr_val.mean(dtype=float).item(), lpips_val.item()
